...
pacman::p_load('ggplot2', 'plyr', 'lme4', 'AICcmodavg', 'MASS', "dplyr")

grass.combined <- read.csv("https://docs.google.com/spreadsheets/d/e/2PACX-1vTS7XPynyjgRkghaf8dMI1k1mPoT9qa1VZNoexYfBvofRjItm3CnYXnIC_Yv5zGF8moqUu-NxRJbMyX/pub?gid=1407150711&single=true&output=csv")

# Rdata created by Devan and sent to me
## How/why did he calculate recovery?
#load("~/Google Drive/data/clip.net.Rdata")
#load("~/Google Drive/data/clip.sum.Rdata")

```


```{r calculate recovery, echo=FALSE, warning=FALSE}
grass.recov <- grass.combined

grass.recov <-  grass.recov %>% 
...
#add REML=false
recov.null <- lmer(recovery~ 1 + (1|block/event), data=recovery.dat)  
recov.spp <- lmer(recovery~ 0 + species + (1|block/event), data=recovery.dat)
recov.treat <- lmer(recovery~ 0 + trt + (1|block/event), data=recovery.dat)
recov.phot <- lmer(recovery ~ 0 + photo + (1|block/event), data=recovery.dat)

recov.add <- lmer(recovery~ 0 + species+trt + (1|block/event), data=recovery.dat)
recov.add2 <- lmer(recovery ~ 0 + photo + trt  + (1|block/event), data=recovery.dat)

recov.int <- lmer(recovery~ 0 + species+ trt + species:trt + (1|block/event), data=recovery.dat)
recov.int2 <- lmer(recovery~ 0 + trt:photo + (1|block/event), data=recovery.dat)
```
...
print(aictab(cand.set = rcv.mods, 
...
rcv.mods.top <- list( )
 ...
 rcv.terms <- c("speciesAGCR", "speciesAGST", "speciesBODA", "speciesBOGR",
	   ...
	   for(i in 1:length(rcv.terms)) {
  	    rcv.av <- modavg(parm = paste(rcv.terms[i]), 
